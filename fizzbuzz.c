...
/* begin main unit */
static void yis_fizzbuzz_fizzbuzz(YisVal);
static void yis_entry(void);
static YisVal yis_fizzbuzz_mod(YisVal v_this, YisVal);
static YisVal __fnwrap_fizzbuzz_fizzbuzz(void*,int,YisVal*);

// cask fizzbuzz
// bring stdr
static YisVal yis_fizzbuzz_mod(YisVal v_this, YisVal v_divisor) {
  return yis_eq(yis_mod(v_this, v_divisor), YV_INT(0));
}

static void yis_fizzbuzz_fizzbuzz(YisVal v_n) {
  { YisVal v_i = YV_INT(1); yis_retain_val(v_i);
  for (; yis_as_bool(yis_le(v_i, v_n)); (void)((yis_move_into(&v_i, yis_add(v_i, YV_INT(1))), v_i))) {
    YisVal v_flag = YV_BOOL(false); yis_retain_val(v_flag);
    if (yis_as_bool(yis_fizzbuzz_mod(v_i, YV_INT(3)))) {
      (void)(stdr_write(YV_STR(stdr_str_lit("Fizz\n"))));
      (void)((yis_move_into(&v_flag, YV_BOOL(true)), v_flag));
    } else if (yis_as_bool(yis_fizzbuzz_mod(v_i, YV_INT(5)))) {
      (void)(stdr_write(YV_STR(stdr_str_lit("Buzz\n"))));
      (void)((yis_move_into(&v_flag, YV_BOOL(true)), v_flag));
    } else if (yis_as_bool(YV_BOOL(!yis_as_bool(v_flag)))) {
      (void)(stdr_write(({ YisVal __ip[2]; __ip[0] = YV_STR(stdr_to_string(v_i)); __ip[1] = YV_STR(stdr_str_lit("\n")); YV_STR(stdr_str_from_parts(2, __ip)); })));
    }
...
static void yis_entry(void) {
#line 23 "/Users/nayu/Developer/Yis/examples/fizzbuzz.yi"
  (void)(stdr_write(YV_STR(stdr_str_lit("== FizzBuzz Example ==\n"))));
#line 24 "/Users/nayu/Developer/Yis/examples/fizzbuzz.yi"
  (void)(yis_fizzbuzz_fizzbuzz(YV_INT(20)));
#line 25 "/Users/nayu/Developer/Yis/examples/fizzbuzz.yi"
  (void)(stdr_write(YV_STR(stdr_str_lit("== Done ==\n"))));
}

static YisVal __fnwrap_fizzbuzz_fizzbuzz(void* env, int argc, YisVal* argv) {
  (void)env; (void)argc;
  YisVal __a0 = argc > 0 ? argv[0] : YV_NULLV;
  yis_fizzbuzz_fizzbuzz(__a0);
  return YV_NULLV;
}
