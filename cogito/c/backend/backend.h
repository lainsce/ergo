...
// Provides platform-agnostic interface for windowing, rendering, input, and text
...
static inline CogitoColor cogito_color(uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
...
typedef struct CogitoBackend {
    ...
    void (*draw_rect_rounded)(int x, int y, int w, int h, CogitoColor color, float roundness);
    void (*draw_rect_lines)(int x, int y, int w, int h, CogitoColor color, int thickness);
    void (*draw_rect_rounded_lines)(int x, int y, int w, int h, CogitoColor color, float roundness, int thickness);
    void (*draw_line)(int x1, int y1, int x2, int y2, CogitoColor color, int thickness);
    void (*draw_circle)(int x, int y, float radius, CogitoColor color);
    void (*draw_circle_lines)(int x, int y, float radius, CogitoColor color, int thickness);
    ...
    // Textures
    CogitoTexture* (*texture_create)(int w, int h, const uint8_t* data, int channels);
    void (*texture_destroy)(CogitoTexture* tex);
    void (*texture_get_size)(CogitoTexture* tex, int* w, int* h);
    void (*draw_texture)(CogitoTexture* tex, CogitoRect src, CogitoRect dst, CogitoColor tint);
    void (*draw_texture_pro)(CogitoTexture* tex, CogitoRect src, CogitoRect dst, CogitoVec2 origin, float rotation, CogitoColor tint);
...
// Get window index
int cogito_window_registry_find(CogitoWindowRegistry* registry, CogitoWindow* window);
...
CogitoWindow* cogito_window_registry_route_event(CogitoWindowRegistry* registry, uint32_t window_id);
